...
namespace Smooth
{
    ...
    public class SmoothSyncNetcode : NetworkBehaviour
    {
        ...
        void UpdateObservers()
        {
            ...
            List<ulong> observersToRemove = new();
            foreach (var kv in previousObservers)
            {
                if (!netIdentity.IsNetworkVisibleTo(kv.Key))
                {
                    observersToRemove.Add(kv.Key);
                }
            }
            foreach (var observerToRemove in observersToRemove)
            {
                previousObservers.Remove(observerToRemove);
            }
        }
